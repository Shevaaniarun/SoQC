...
export default function CreateArticle({
  ...
  // Pre-populate fields if editing an existing article
  useEffect(() => {
    if (isEditing && id) {
      // 1. Fetch user articles from local storage and combine with prop articles
      const savedArticles: Article[] = JSON.parse(localStorage.getItem('userArticles') || '[]')
      const combinedArticles = [...savedArticles, ...articles]
      
      const existing = combinedArticles.find((art) => String(art.id) === String(id))
      if (existing) {
        setOriginalArticle(existing)
        setTitle(existing.title || '')
        setCategory(existing.category || categories[0])
        setExcerpt(existing.excerpt || '')
        setContent(existing.content || existing.excerpt || '')
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      // Fetch existing user-created articles from localStorage
      const savedArticles: Article[] = JSON.parse(localStorage.getItem('userArticles') || '[]')

      if (isEditing && id) {
        const updatedArticle: Article = {
          id: originalArticle ? originalArticle.id : id,
          title,
          category,
          excerpt: excerpt || content.slice(0, 120) + '...',
          content,
          author,
          date: originalArticle?.date || new Date().toISOString().split('T')[0],
          readTime: calculateReadTime(content),
          image: imageUrl || 'https://images.unsplash.com/photo-1635070041078-e363dbe005cb?auto=format&fit=crop&q=80&w=800',
          status: user?.role === 'admin' ? (originalArticle?.status || 'approved') : 'pending',
        }

        // Update in localStorage
        const updatedList = savedArticles.map((art) =>
          String(art.id) === String(updatedArticle.id) ? updatedArticle : art
        )
        if (!savedArticles.some((art) => String(art.id) === String(updatedArticle.id))) {
          updatedList.unshift(updatedArticle)
        }
        localStorage.setItem('userArticles', JSON.stringify(updatedList))
        ...
        // Save new article to localStorage so ArticleDetail can read it
        const updatedList = [newArticle, ...savedArticles]
        localStorage.setItem('userArticles', JSON.stringify(updatedList))

        if (onArticleCreated) {
          onArticleCreated(newArticle)
        }
        toast.success(user?.role === 'admin' ? 'Article published!' : 'Submitted for review!')
      }
      ...
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '0 24px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32 }}>
          ...
          <span style={{ fontFamily: 'JetBrains Mono', color: '#a855f7', fontSize: 12, letterSpacing: '0.15em', textTransform: 'uppercase' }}>
            {isEditing ? 'Editing Portal' : 'Publishing Portal'}
          </span>
        </div>
        ...
        >
          ...
          {user?.role !== 'admin' && (
            <div style={{
              ...
              ℹ️ <strong>Submission Info:</strong> Articles created or modified by non-admin accounts will be placed in a moderation queue for approval before being publicly displayed.
            </div>
          )}
