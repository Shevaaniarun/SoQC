    ...
    <>
      ...
      <AnimatePresence mode="wait">
        <Routes location={location} key={location.pathname}>
          ...
          {/* CONTINUE / ROLE CHECKPOINT ROUTE */}
          <Route
            path="/continue"
            element={
              <PageTransition>
...
function AppContent({ isMobile }: { isMobile: boolean }) {
  ...
  // 1. MANAGE USER AUTH STATE WITH LOCALSTORAGE PERSISTENCE
  const [user, setUser] = useState<{ name: string; role: 'user' | 'admin' } | null>(() => {
    const saved = localStorage.getItem('soqc_user')
  ...
  // LOGOUT FUNCTION
  const handleLogout = () => {
    localStorage.removeItem('soqc_user')
    setUser(null)
    toast.success('Logged out successfully')
  }

  // 2. INITIALIZE ARTICLES WITH LOCALSTORAGE PERSISTENCE
  const [articles, setArticles] = useState(() => {
    const savedArticles = localStorage.getItem('soqc_articles')
    if (savedArticles) {
      return JSON.parse(savedArticles)
    }
    return initialArticles.map((art: any) => ({
      ...art,
  ...
  useEffect(() => {
    localStorage.setItem('soqc_articles', JSON.stringify(articles))
  }, [articles])

  // 3. USER ACTION: CREATE ARTICLE
  const handleAddArticle = (newArticle: any) => {
    ...
    setArticles((prevArticles: any[]) => [articleWithStatus, ...prevArticles])

    if (user?.role === 'admin') {
      toast.success('Article published directly!')
    } else {
      toast.success('Article submitted for admin review!')
    }
  }

  // 4. USER/ADMIN ACTION: UPDATE EXISTING ARTICLE
  ...
  const handleRejectArticle = (id: string | number) => {
    setArticles((prev: any[]) => prev.filter((art) => art.id !== id))
  ...
  return (
    <>
      <div style={{ minHeight: '100vh', background: '#03030f', position: 'relative' }}>
        ...
        {/* 1. TOP-LEFT CONTAINER: USER INFO & LOG OUT ONLY */}
        {user && (
          <div
            style={{
              position: 'fixed',
        ...
        {/* 2. TOP-RIGHT CONTAINER: SOUND TOGGLE ONLY */}
        <div
          style={{
            position: 'fixed',
            top: 20,
        ...
        <main style={{ position: 'relative', zIndex: 1, minHeight: '100vh' }}>
          <AnimatedRoutes
            ...
            onArticleUpdated={handleUpdateArticle}
